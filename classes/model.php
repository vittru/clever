...
Class Model {
    ...
    function getCategory($id) {
        ...
        $sqlSelect->closeCursor();
        if ($data) {
            $category = new Category($data['id'], $data['name'], $data['description'], $data['url'], $data['metaTitle'], $data['metaDescription'], $data['metaKeywords'], $data['supercatid']);
        }
        return $category;
    }
    ...
    function getSuperCats() {
        ...
        while ($data = $sqlSelect->fetch(PDO::FETCH_ASSOC)) {
            $category = New Category($data['id'], $data['name'], $data['description'], $data['url'], $data['metaTitle'], $data['metaDescription'], $data['metaKeywords'], NULL);
            if (!isset($categories))
                $categories = [$category];
            else
                array_push($categories, $category);
